const emtyArrgs = "Content cannot be left blank"; // tin nhắn trả về khi người dùng nhập thiếu dữ liệu 
const sendWaiting = true; // bật hoặc tắt gửi tin nhắn "đang tạo hình ảnh, vui ồng chờ đợi...";
const textWaiting = "Processing please wait";
  ...
  async function CheckExistAndDownload(link, dir) {
    if (!existsSync(dir)) {
      try {
        ...
        sendMessage("Sorry, there was an error during the download", threadID, messageID);
      }
    }
  }
